...
describe 'Issue page requests' do
  ...
  shared_examples_for 'a page request' do
  ...
  describe Hexabat::RequestCreator do
    ...
    before  { http.stub(:get).and_return(get) }

    it_behaves_like 'a page request'
  end

  describe Hexabat::TokenAuthorizedRequestCreator do
    subject     { described_class.new repository, token, &callback }
    let(:token) { 'xxxx' }

    before do
      headers = { 'Authorization' => "token #{token}" }
      query   = { page: 1, per_page: 100, state: 'open' }
